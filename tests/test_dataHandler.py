...
class TestDataHandler(TestCase):
    ...
    def test_load_from_file(self):
        ...
        self.assertEquals(8, len(self.dataHandler._train_set))
        self.assertEquals(2, len(self.dataHandler._test_set))

    def test_split_data(self):
        self.dataHandler.split_data(0.5)
        self.assertListEqual([10.0, 20.0, 20.0], self.dataHandler._train_set[0])
        self.assertListEqual([60.0, 60.0, 60.0], self.dataHandler._test_set[0])
        self.assertEquals(5, len(self.dataHandler._train_set))
        self.assertEquals(5, len(self.dataHandler._test_set))
